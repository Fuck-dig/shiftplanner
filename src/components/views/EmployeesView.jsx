  ...
  <div style={{display:'flex',flexDirection:'column',gap:10}}>
      ...
      {expandedEmp===emp.id&&(<div style={{marginTop:18,paddingTop:18,borderTop:`1px solid ${T.border}`}}>
        ...
        <div style={{background:T.surfaceWarm,border:`1px solid ${T.border}`,borderRadius:10,padding:'12px 14px',marginBottom:12}}>
          ...
          <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:6,alignItems:'flex-start'}}>
            ...
            {/* Blank = inherit the restaurant default; 0 = this person gets
                nothing. Those are different answers, so the empty string is
                stored as null rather than coerced to a number — see
                effectiveSickPct and the data layer. The placeholder shows what
                inheriting currently means, so blank is never a mystery. */}
            <div style={{flex:'1 1 110px'}}><div style={{fontSize:11,color:T.text3,marginBottom:4}}>{t('emp.sickPay')}</div><div style={{display:'flex',alignItems:'center',gap:5}}><input type="number" min="0" max="100" step="1" placeholder={orgSickPct==null?'':String(orgSickPct)} value={emp.sickPayPct??''} onChange={e=>{const v=e.target.value;updateEmp(emp.id,'sickPayPct',v===''?null:Math.max(0,Math.min(100,Number(v))));}} style={{...s.input,flex:1}}/><span style={{fontSize:11,color:T.text3,flexShrink:0}}>%</span></div><div style={{fontSize:10,color:T.text3,marginTop:3}}>{emp.sickPayPct==null?t('emp.sickPayInherit'):t('emp.sickPayOwn')}</div></div>
    ...
    {showAddEmp&&(<div style={s.card}>
      ...
      <div style={{display:'flex',gap:8,marginBottom:10,flexWrap:'wrap'}}>
        ...
        <div style={{flex:'2 1 200px'}}><SectionLabel mb={4}>{t('emp.roles')}</SectionLabel><div style={{display:'flex',gap:4,flexWrap:'wrap'}}>{allRoles.map(r=>{const active=(newEmp.roles||[]).includes(r),rs=roleStyles[r]||DEFAULT_ROLE_STYLES.Other;return<button key={r} onClick={()=>{const cur=newEmp.roles||[];const next=active?cur.filter(x=>x!==r):[...cur,r];if(next.length>0)setNewEmp(p=>({...p,roles:next}));}} style={{display:'inline-flex',alignItems:'center',gap:4,padding:'4px 9px',borderRadius:999,fontSize:11,fontWeight:500,background:active?(isDark()?rs.dot+'22':rs.bg):'transparent',color:active?(isDark()?rs.dot:rs.text):T.text3,border:`1px solid ${active?(isDark()?rs.dot+'55':rs.border):T.border}`,cursor:'pointer',fontFamily:'inherit'}}><span style={{width:5,height:5,borderRadius:'50%',background:active?rs.dot:T.text3}}/>{r}</button>;})}
        </div></div>
      </div>
