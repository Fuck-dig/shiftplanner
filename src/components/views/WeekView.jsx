                ...
                return(<div key={row.empId} style={{position:'relative',height:ganttRowH,background:T.surfaceWarm,borderRadius:6}}>
                  {row.merged.map((seg,si)=>{
                    ...
                    return(<Fragment key={si}>
                      ...
                      <div onClick={()=>{if(ganttJustDraggedRef.current)return;openEditSlot(effectiveDay,seg.blockId,segIdx);}} title={t('week.editShift')} style={{position:'absolute',left:`${leftPct}%`,width:`${widthPct}%`,top:0,bottom:0,minWidth:14,background:isDark()?barColor+'40':barColor+'30',border:`1.5px solid ${barColor}`,borderRadius:6,zIndex:dragging?5:1,boxShadow:dragging?'0 2px 8px rgba(0,0,0,0.25)':'none',cursor:'pointer'}}>
                        {/* Centered on the bar's own midpoint, but NOT clipped
                            to its width and given its own background — a very
                            short actual bar (e.g. clocking in and out within
                            the same minute) would otherwise squeeze this text
                            down to nothing. It can spill past the bar's edges
                            and stay fully readable. */}
                        <span style={{position:'absolute',left:'50%',top:'50%',transform:'translate(-50%,-50%)',fontSize:isMobile?9:10,fontWeight:600,color:isDark()?barColor:isNoShow?barColor:rs.text,whiteSpace:'nowrap',padding:'1px 6px',borderRadius:4,background:isDark()?'rgba(30,24,20,0.7)':'rgba(255,255,255,0.85)',pointerEvents:'none',zIndex:3}}>{label}</span>
